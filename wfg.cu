...
/* myFirstKernel */
__global__ void myFirstKernel(FRONT *d_a, FRONT front, int index) {
	d_a->points[blockIdx.x].objectives[threadIdx.x] = (OBJECTIVE) MIN(front.points[index].objectives[threadIdx.x], front.points[blockIdx.x+1+index].objectives[threadIdx.x]);
}
...
void limitset(int index, FRONT front) {
	...
	if (iteration > maxDepth) {
		...
		fr[iteration].points = (POINT *) malloc(sizeof(POINT) * maxPoints);
		for (int j = 0; j < maxPoints; j++) {
			//only need to malloc for n (number of Dimensions)
			fr[iteration].points[j].objectives = (OBJECTIVE *) malloc(sizeof(OBJECTIVE) * n);
		}
	}
	...
	FRONT *d_a;
	cudaMalloc( (void **) &d_a, 100000);
	FRONT *h_a = (FRONT *) malloc(100000);
	...
	myFirstKernel<<< z, n >>>( d_a, front, index);
	...
	cudaMemcpy( h_a, d_a, 100000, cudaMemcpyDeviceToHost);
	fr[iteration] = (FRONT) *h_a;
	...
	for (int i = 1; i < z; i++) {
		bool add = true;		//whether to store the point in memory

		for (int j =0; j < fr[iteration].nPoints && add; j++) {
...
/*int compare (const void *a, const void *b)
{
	...
	if (((*(POINT *)a).objectives[n-1] > (*(POINT *)b).objectives[n-1]))
		return 1;
	if (((*(POINT *)a).objectives[n-1] < (*(POINT *)b).objectives[n-1]))
		return -1;
	return 0;
...
int main(int argc, char *argv[]) {
	...
	//fr = malloc(size1of(FRONT) * maxPoints);
	fr = (FRONT *) malloc(sizeof(FRONT) * maxDimensions);
