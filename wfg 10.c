...
double ehv(int index, FRONT front) {
	...
	//if not the first point, then go deeper into the recursion
...
double hypervolume(FRONT front) {
	...
	for (int i = front.nPoints - 1; i >= 0; i--) //decreasing index!
...
int main(int argc, char *argv[]) {
	...
	for (int i = 0; i < maxd; i++) {
		...
		for (int j = 0; j < maxPoints; j++) {
		//only need to malloc for n (number of Dimensions)
			fr[i].points[j].objectives = malloc(sizeof(OBJECTIVE) * (maxDimensions - (i+1))); //sliced
		}
	}
