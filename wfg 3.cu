...
void limitset(int index, FRONT front) {
	...
	int *h_scanoutput = (int *) malloc((z)*sizeof(int));
	
	scan_best<<< 256, 512/2, sizeof(int)*(512) >>>(d_scanbest, d_eliminated, 512);
	
	cudaThreadSynchronize();
	printf("z = %d\n", z);
	cudaMemcpy(h_scanoutput, d_scanbest, (z)*sizeof(int), cudaMemcpyDeviceToHost);
	printf("scnbes: ");
	for (int i = 0 ; i < z; i++) {
