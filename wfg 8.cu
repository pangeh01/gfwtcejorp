...
// 6) Make into single kernel function with many device codes, single cudaThreadsynchronise (implement scan_best in device mode) - don't know how to make it work
...
double *d_fr;	//storage for storing array of sprimed/non-dominating fronts as we go deeper into the recursion
...
double ehv(int index, FRONT front) {
	...
	if (index < front.nPoints-1) {
		...
		iteration++;	//slicing
		ehv -= hypervolume(d_fr[(iteration-1)*maxPoints*maxDimensions*sizeof(double)]);	//subtract the hypervolume of the limit set from ehv.
		iteration--;
	}
...
/**
 * Returns the size of hypervolume of a front. Iterative version.
 */
void hv_iterative()
// sets vs[0] to the hypervolume of fs[0][0 ..] 
{
  qsort(fs[0].points, fs[0].nPoints, sizeof(POINT), greater);
  ps[0] = fs[0].nPoints - 1; 
  while (ps[0] >= 0) // there are jobs remaining 
    if (ps[fr] < 0) // we've finished the jobs at this level 
      {fr--; 
       evs[fr] -= vs[fr+1]; 
       vs[fr] += fabs(fs[fr].points[ps[fr]].objectives[n] - ref.objectives[n]) * evs[fr];
       ps[fr]--;
       n++;}
    else
    if (n == 2) // do this job using the linear algorithm 
      {fr--;
       evs[fr] -= fabs((fs[fr+1].points[0].objectives[0] - ref.objectives[0]) * 
                       (fs[fr+1].points[0].objectives[1] - ref.objectives[1])); 
       for (int i = 1; i < fs[fr+1].nPoints; i++) 
         evs[fr] -= fabs((fs[fr+1].points[i].objectives[0] - ref.objectives[0]) * 
                         (fs[fr+1].points[i].objectives[1] - fs[fr+1].points[i-1].objectives[1]));
       vs[fr] += fabs(fs[fr].points[ps[fr]].objectives[n] - ref.objectives[n]) * evs[fr];
       ps[fr]--;
       n++;}
      ...
      {n--;
       evs[fr] = 1;
       for (int i = 0; i < n; i++) 
         evs[fr] *= fabs(fs[fr].points[ps[fr]].objectives[i] - ref.objectives[i]);
       if (ps[fr] == fs[fr].nPoints - 1) // first job at this level: set will be empty 
         {vs[fr] = fabs(fs[fr].points[ps[fr]].objectives[n] - ref.objectives[n]) * evs[fr];
          ps[fr]--;
          n++;}
         ...
         {fr++; 
          makeDominatedBit(); 
          qsort(fs[fr].points, fs[fr].nPoints, sizeof(POINT), greater);
          ps[fr] = fs[fr].nPoints - 1;}}
}
...
// allocate memory
  fs = malloc(sizeof(FRONT) * (maxn /* - 1 */)); 
  for (int i = 1; i < maxn /* - 1 */; i++) 
    {fs[i].points = malloc(sizeof(POINT) * maxm); 
     for (int j = 0; j < maxm; j++) 
       fs[i].points[j].objectives = malloc(sizeof(OBJECTIVE) * (maxn /* - (i + 1) */)); 
    }
  ps  = malloc(sizeof(int)    * (maxn /* - 2 */));
  evs = malloc(sizeof(double) * (maxn /* - 2 */));
  vs  = malloc(sizeof(double) * (maxn /* - 1 */));

	int maxd = maxDimensions-2;
	//allocate an array of fronts for nDimensions
	cudaMalloc( (void **) &d_fr, maxd*maxPoints*maxDimensions*sizeof(double));
